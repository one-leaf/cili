...
**This skill MUST be delegated to a Worker — the main agent must NOT execute it directly!**

### Execution flow:

1. **After reading this skill, immediately delegate the research task via the `agent` tool.**
...
### Why delegate to a Worker?

- **Isolation**: Research may involve heavy searching and verification — avoid polluting the main conversation context.
- **Focus**: The Worker can execute the 5-step closed loop without distraction from other tasks.
...
**Important**: After triggering, the agent must read this skill, then immediately delegate via the `agent` tool.

---

## Execution Protocol (5-step closed loop, executed by Worker)

Upon receiving the research task, the Worker must execute the following flow:
