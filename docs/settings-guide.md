...
### mineru_api_key — MinerU PDF 解析 API 密钥

**作用**：用于高精度 PDF 转 Markdown 的 API 密钥。MinerU 是一个专业的文档解析服务。

**默认值**：`""`（空字符串，表示不使用该功能）

**说明**：

- 如果不使用 MinerU 服务，留空即可
- Agent 处理 PDF 时会使用内置的普通解析方式
- 如果需要更高质量的 PDF 解析（特别是含表格、公式的文档），可以注册 MinerU 并填写 API Key
